...
from .models import ShelterAccount, AnimalCard, ShelterNews, LostAnimals, ShelterReport, LostAnimals, TakeAnimal, \
    ShelterHotReport, AnimalReport, UserAccount
...
class LoginForm(forms.Form):
    ...
    account_type = forms.ChoiceField(initial="shelter", choices=(("user", "user"), ("shelter", "shelter")), widget=forms.HiddenInput)
...
class AddRegisterForm(ModelForm):
    ...
    class Meta:
        model = ShelterAccount
        fields = ['about', 'director_name', 'contact', 'choices_type_card', 'requisites', 'social_network',
                  'number_of_animals', 'logo']
        labels = {
            'about': 'Опишите приют',
...
class ChangeCardAnimalForm(ModelForm):
    class Meta:
        model = AnimalCard
        fields = ['name', 'comment', 'summ', 'photo', 'video', 'status', 'breed', 'gender', 'age']
        labels = {
